...
class GINConv(nn.Module):
    def __init__(self,feats):
        ...
        self.mlp = torch.nn.Sequential(nn.Linear(feats, 2 * feats), nn.BatchNorm1d(2 * feats),
                                    nn.ReLU(), nn.Linear(2 * feats, feats))
    ...
    def forward(self, graph, nfeat, efeat):
        ...
        graph.edata['e'] = F.relu(edge_embedding + graph.edata['e'])
        graph.update_all(fn.copy_edge('e', 'm'), fn.sum('m', 'ft'))

        rst = graph.ndata['ft']
        rst = self.mlp(rst + (1 + self.eps) * nfeat)
...
# GIN model
class GIN(nn.Module):
    r"""GIN from `Open Graph Benchmark: Datasets for Machine Learning on Graphs
    <https://arxiv.org/abs/2005.00687>`__
    ...
    def __init__(self,
        ...
        for i in range(n_layers):
            self.layers.append(GINConv(n_hidden))
